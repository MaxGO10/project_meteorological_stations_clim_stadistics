...
unique(substr(colnames(pre[,-1]),1,10)) #para selecionar las estaciones unicas
...
#para quitar la columna de fecha
pre <- data.frame(pre[,-1])

##Reemplazar por estaciones del grupo
#
tmax <-data.frame(tmax[,-1][1:3],ho00000780=NA,tmax[,-1][4:9]) 
...
#se crea el archivo csv en el formato, año
estacion<- list()
...
files <- c("ho00000776.csv","ho00000778.csv","ho00000779.csv","ho00000780.csv","ho00000781.csv","ho00000783.csv","ho00000785.csv","ho00000786.csv","ho00000787.csv","ho00000788.csv")
estacion <- list()
for (k in 1:10){
...
for (j in 1:length(funciones)){
  ...
  for (i in 1:10){ 
    ...
    if (i==length(estacion)){
      colnames(index_df2) <-  c("AYAVIRI","PROGRESO","LAMPA","CABANILLAS","AZANGARO","ARAPA","MUNANI","HUANCANE","HUARAYAMOHO","CAPACHICA")
      ...
      if (nrow(index_df2 > 55)){
        ...
        graph <- ggplot(melt(index_df2, id.vars = "Fecha",variable.name = "Estacion"), aes(x = Fecha, y = value)) +
          ...
          scale_color_manual(values = c("red", "blue", "green","black","violet","orange",
                                        "cyan4","yellow","brown","darkorange"))+ ylab(toupper(substr(funciones[[1]],9,nchar(funciones[[1]]))))+ggtitle(paste0("Índice ",Name_index," 1965 - 2019"))+
          theme_bw()+
        ...
        graph <- ggplot(melt(index_df2, id.vars = "Fecha",variable.name = "Estacion"), aes(x = Fecha, y = value)) +
          ...
          facet_wrap(~ Estacion, ncol = 1, scales = "free_y")+
          scale_color_manual(values = c("red", "blue", "green","black","violet","orange",
                                        "cyan4","yellow","brown","darkorange"))+ ylab(toupper(substr(funciones[[j]],9,nchar(funciones[[j]]))))+ggtitle(paste0("Índice ",Name_index," 1965 - 2019"))+
          theme_bw()+
          theme(plot.title = element_text(hjust = 0.5,face = "bold"),
                strip.background = element_rect(fill = "cadetblue1"),  strip.text = element_text(color = "black", face = "bold"))+
          scale_x_date(date_labels = "%Y", date_breaks = "3 year")+
...
graph <- ggplot(melt(index_df2, id.vars = "Fecha",variable.name = "Estacion"), aes(x = Fecha, y = value)) +
  ...
  facet_wrap(~ Estacion, ncol = 1, scales = "free_y")+
  scale_color_manual(values = c("red", "blue", "green","black","violet","orange",
                                "cyan4"))+ ylab(toupper(substr(funciones[[1]],9,nchar(funciones[[1]]))))+ggtitle(paste0("Índice ",Name_index," 1965 - 2019"))+
  theme(plot.title = element_text(hjust = 0.5,face = "bold"),
                                  strip.background = element_rect(fill = "cadetblue1"),  strip.text = element_text(color = "black", face = "bold"))+
scale_x_date(date_labels = "%Y", date_breaks = "3 year")+
  theme(axis.title = element_text(face = "bold"))
